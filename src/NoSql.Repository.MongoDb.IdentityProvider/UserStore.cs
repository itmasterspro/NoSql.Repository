...
namespace ItMastersPro.NoSql.Repository.MongoDb.Identity
{
    ...
    /// </summary>
    public class UserStore : UserStore<Identity.IdentityUser>
    {
        /// <summary>
        ...
        public UserStore(IMongoDbContext context, IdentityErrorDescriber describer = null) : base(context, describer) { }
    }
    ...
    public class UserStore<TUser> : UserStore<TUser, Identity.IdentityRole, IMongoDbContext, ObjectId>
        where TUser : Identity.IdentityUser,  new()
    {
        /// <summary>
    ...
    public class UserStore<TUser, TRole, TContext> : UserStore<TUser, TRole, TContext, ObjectId>
        where TUser : Identity.IdentityUser, IEntity
        where TRole : Identity.IdentityRole, IEntity
    ...
    public class UserStore<TUser, TRole, TContext, TKey> : UserStore<TUser, TRole, TContext, Identity.IdentityUserClaim, Identity.IdentityUserRole, Identity.IdentityUserLogin, Identity.IdentityUserToken, Identity.IdentityRoleClaim>
    ...
    public class UserStore<TUser, TRole, TContext, TUserClaim, TUserRole, TUserLogin, TUserToken, TRoleClaim> :
        NoSql.Repository.MongoDb.Identity.UserStoreBase<TUser, TRole, TUserClaim, TUserRole, TUserLogin, TUserToken, TRoleClaim>,
        IProtectedUserStore<TUser>
        where TUser : NoSql.Repository.MongoDb.Identity.IdentityUser
        where TRole : Identity.IdentityRole
        where TContext : IMongoDbContext
        where TUserClaim : Identity.IdentityUserClaim, new()
        where TUserRole : Identity.IdentityUserRole, new()
        where TUserLogin : Identity.IdentityUserLogin, new()
        where TUserToken : Identity.IdentityUserToken, new()
        where TRoleClaim : Identity.IdentityRoleClaim, new()
    {
        /// <summary>
        ...
        private IMongoDbRepository<TUser> UsersSet => Context.Set<TUser>().Result;
        private IMongoDbRepository<TRole> Roles => Context.Set<TRole>().Result;
        private IMongoDbRepository<TUserClaim> UserClaims => Context.Set<TUserClaim>().Result;
        private IMongoDbRepository<TUserRole> UserRoles => Context.Set<TUserRole>().Result;
        private IMongoDbRepository<TUserLogin> UserLogins => Context.Set<TUserLogin>().Result;
        private IMongoDbRepository<TUserToken> UserTokens => Context.Set<TUserToken>().Result;
        ...
        public override async Task<IdentityResult> CreateAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await UsersSet.InsertAsync(user, cancellationToken);
        ...
        public override async Task<TUser> FindByIdAsync(string userId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return await UsersSet.FindAsync(u=>u.Id == userId.ToObjectId(), cancellationToken);
        }
        ...
        public override async Task<TUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return await Task.Run(()=> Users.FirstOrDefault(u => u.NormalizedUserName == normalizedUserName), cancellationToken);
        }

        /// <summary>
        /// A navigation property for the users the store contains.
        /// </summary>
        public override IQueryable<TUser> Users => UsersSet.Query();
        ...
        {
            return await Roles.FindAsync(r => r.NormalizedName == normalizedRoleName, cancellationToken);
        }
        ...
        {
            return await UserRoles.FindAsync(ur => ur.UserId == userId && ur.RoleId == roleId, cancellationToken);
        }
        ...
        {
            return await UsersSet.FindAsync(u => u.Id.Equals(userId), cancellationToken);
        }
        ...
        protected override Task<TUserLogin> FindUserLoginAsync(ObjectId userId, string loginProvider, string providerKey, CancellationToken cancellationToken)
        {
            return UserLogins.FindAsync(userLogin => userLogin.UserId.Equals(userId) && userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey, cancellationToken);
        }
        ...
        {
            return UserLogins.FindAsync(userLogin => userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey, cancellationToken);
        }
        ...
        public override async Task RemoveFromRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (roleEntity != null)
            {
                ...
                if (userRole != null)
                {
                    await UserRoles.DeleteAsync(userRole);
                }
            }
        }
        ...
        public override async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = user.Id;
            var rolesIds = UserRoles.Query().Where(ur => ur.UserId == userId).Select(ur=>ur.RoleId).ToList();
        ...
        public async override Task<IList<Claim>> GetClaimsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return UserClaims.Query(uc => uc.UserId.Equals(user.Id)).Select(c => c.ToClaim()).ToList();
        }
        ...
        public override Task AddClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                UserClaims.UpdateAsync(CreateUserClaim(user, claim));
            }
        ...
        public async override Task ReplaceClaimAsync(TUser user, Claim claim, Claim newClaim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var matchedClaims = UserClaims.Query(uc => uc.UserId.Equals(user.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type).ToList();
        ...
        public async override Task RemoveClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            foreach (var claim in claims)
            {
                ...
                {
                    await UserClaims.DeleteAsync(c);
                }
            }
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            UserLogins.UpdateAsync(CreateUserLogin(user, login));
            return Task.FromResult(false);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (entry != null)
            {
                await UserLogins.DeleteAsync(entry);
            }
        }
        ...
        public async override Task<IList<UserLoginInfo>> GetLoginsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = user.Id;
            return UserLogins.Query(l => l.UserId.Equals(userId))
        ...
        public override async Task<TUser> FindByEmailAsync(string normalizedEmail, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return await UsersSet.FindAsync(u => u.NormalizedEmail == normalizedEmail, cancellationToken);
        }
        ...
        public async override Task<IList<TUser>> GetUsersForClaimAsync(Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var query = from userclaims in UserClaims.Query()
        ...
        public async override Task<IList<TUser>> GetUsersInRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (role != null)
            {
                var usersIdsInRole = UserRoles.Query(ur => ur.RoleId == role.Id).Select(ur=>ur.UserId).ToList();
                return Users.Where(u => usersIdsInRole.Contains(u.Id)).ToList();
            }
        ...
        {
            return UserTokens.FindAsync(ut=>ut.UserId == user.Id && ut.LoginProvider == loginProvider && ut.Name == name, cancellationToken);
        }
