...
namespace ItMastersPro.NoSql.Repository.MongoDb.Identity
{
    ...
    public class RoleStore<TRole> : RoleStore<TRole, IMongoDbContext>
        where TRole : Identity.IdentityRole
    {
        ...
        public RoleStore(IMongoDbContext context, IdentityErrorDescriber describer = null) : base(context, describer) { }
    }
    ...
    public class RoleStore<TRole, TContext> : RoleStore<TRole, TContext, Identity.IdentityUserRole, Identity.IdentityRoleClaim>,
    ...
    public class RoleStore<TRole, TContext, TUserRole, TRoleClaim> :
        ...
        where TRoleClaim : Identity.IdentityRoleClaim, IEntity, new()
    {
        /// <summary>
        ...
        public virtual async Task<IdentityResult> CreateAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await RolesContext.InsertAsync(role, cancellationToken);
        ...
        public virtual async Task<IdentityResult> UpdateAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await RolesContext.UpdateAsync(role);
            return IdentityResult.Success;
        }
        ...
        public virtual async Task<IdentityResult> DeleteAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await RolesContext.DeleteAsync(role);
            return IdentityResult.Success;
        }
        ...
        public virtual Task<TRole> FindByIdAsync(string id, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var roleId = id.ToObjectId();
            return Task.Run(()=> Roles.FirstOrDefault(u => u.Id.Equals(roleId)), cancellationToken);
        }
        ...
        public virtual Task<TRole> FindByNameAsync(string normalizedName, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return Task.Run(()=>Roles.FirstOrDefault(r => r.NormalizedName == normalizedName), cancellationToken);
        }
        ...
        public async virtual Task<IList<Claim>> GetClaimsAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return RoleClaims.Query().Where(rc => rc.RoleId.Equals(role.Id)).Select(c => new Claim(c.ClaimType, c.ClaimValue)).ToList();
        }
        ...
        public virtual Task AddClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            RoleClaims.UpdateAsync(CreateRoleClaim(role, claim));
        ...
        public virtual async Task RemoveClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var claims = RoleClaims.Query().Where(rc => rc.RoleId.Equals(role.Id) && rc.ClaimValue == claim.Value && rc.ClaimType == claim.Type).ToList();
            foreach (var c in claims)
            {
                await RoleClaims.DeleteAsync(c);
            }
        }
        ...
        /// </summary>
        public virtual IQueryable<TRole> Roles => Context.Set<TRole>().Result.Query();

        public IMongoDbRepository<TRole> RolesContext => Context.Set<TRole>().Result;

        private IMongoDbRepository<TRoleClaim> RoleClaims { get { return Context.Set<TRoleClaim>().Result; } }
